...
subplot(231);
data = twospirals();
csvwrite('twospirals.csv', data);
scatter(data(:,1), data(:,2), dotsize, data(:,3)); axis equal;
title('Two spirals');

subplot(232);
data = clusterincluster();
csvwrite('clusterincluster.csv', data);
scatter(data(:,1), data(:,2), dotsize, data(:,3)); axis equal;
title('Cluster in cluster');

subplot(233);
data = corners();
csvwrite('corners.csv', data);
scatter(data(:,1), data(:,2), dotsize, data(:,3)); axis equal;
title('Corners');

subplot(234);
data = halfkernel();
csvwrite('halfkernel.csv', data);
scatter(data(:,1), data(:,2), dotsize, data(:,3)); axis equal;
title('Half-kernel');

subplot(235);
data = crescentfullmoon();
csvwrite('crescentfullmoon.csv', data);
scatter(data(:,1), data(:,2), dotsize, data(:,3)); axis equal;
title('Crescent & Full Moon');

subplot(236);
data = outlier();
csvwrite('outlier.csv', data);
scatter(data(:,1), data(:,2), dotsize, data(:,3)); axis equal;
title('Outlier');